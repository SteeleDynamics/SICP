...
#|
 ...
 | Answer
 |
 | Option #1: translation in exercise 4.18
 |
 | (lambda (f y0 dt)
 |   (let ((y '*unassigned*)
 |         (dy '*unassigned*))
 ...
 |     ⟨e3⟩))
 |
 | Evaluating (a (integral (delay dy) y0 dt)) is successful since delay is a
 | special form such that it doesn't evaluate its argument. However, when we
 | evaluate (b (stream-map f y)), we see that y has the value '*unassigned*.
 | This results in an unbound variable error. This procedure will NOT work.
 |
 | Option #2: translation in §4.1.6
 |
 | (lambda (f y0 dt)
 |   (let ((y '*unassigned*)
 |         (dy '*unassigned*))
 ...
 |     y))
 |
 | Evaluating (set! y (integral (delay dy) y0 dt)) is successful since delay is
 | a special form such that it doesn't evaluate its argument. When we evaluate
 | (set! dy (stream-map f y)), y has a value that is NOT '*unassigned*. This
 | procedure WILL work.
 |#
