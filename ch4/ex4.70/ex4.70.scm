...
#|
 ...
 | 'ones' in section 3.5.2: '(define ones (cons-stream 1 ones))'.
 |
 | (define (add-assertion! assertion)
 |   (store-assertion-in-index assertion)
 |   (set! THE-ASSERTIONS
 ...
 | Answer
 |
 | Q1. What is the purpose of the 'let' bindings in the procedures
 |     'add-assertion!' and 'add-rule!'?
 |
 | A1. The purpose is to capture the old value of 'THE-ASSERTIONS'.
 |     'Cons-stream' delays evaluation of its second argument. Therefore,
 |     'THE-ASSERTIONS' will have the new value (itself) when applying the
 |     procedure 'stream-cdr'.
 |
 | Q2. What would be wrong with the following implementation of
 |     'add-assertion!'?
 |
 | A2. From A1, 'THE-ASSERTIONS' becomes an infinite stream of only the new
 |     assertion, not a stream where the new assertion is 'cons-stream'ed onto
 |     the old 'THE-ASSERTIONS' stream and then updated.
 |#
