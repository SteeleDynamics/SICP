...
#|
 ...
 | them from left to right. Explain why our parsing program wouldn't work if
 | the operands were evaluated in some other order.
 |
 | Answer
 |
 | Consider the evaluation of the expression where operands are evaluated
 | from right to left:
 |
 | Expression                                   *unparsed* pointer
 | ##########################################################################
 | (parse '(The cat sleeps in the class))       '()
 |
 | (parse-sentence)                             (The cat sleeps in the class)
 |
 | (list 'sentence                              (The cat sleeps in the class)
 |       (parse-noun-phrase)
 |       (parse-verb-phrase))
 |
 | (list 'sentence                              (The cat sleeps in the class)
 ...
 |       (maybe-extend (parse-word verbs)))
 |
 | ==> Fails without evaluating an 'amb' expression to try other choices
 | ==> 'amb' evaluator will indicate that there are no other solutions
 |#
