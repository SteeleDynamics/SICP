...
; make-named-let constructor procedure
...
; while-pred selector procedure
(define (while-pred exp) (cadr exp))

; while-pred selector procedure
(define (while-body exp) (caddr exp))

; while->named-let procedure
(define (while->named-let exp)
  (make-named-let
   'loop
   (zip (list 'pred 'body)
        (list (make-lambda '() (list (while-pred exp)))
              (make-lambda '() (list (while-body exp)))))
   (list (make-if '(pred)
                  (make-begin '((body) (loop pred body)))
                  ''ok))))

(define E0 (setup-environment))
(define exp '(while (< x 8) (begin (newline) (display x) (set! x (+ x 1)))))
(while->named-let exp)
(eval '(define x 0) E0)
(eval exp E0)
