...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)                                     ; clause from
         (apply (eval (operator exp) env)         ;!            ; book
                (operands exp)
                env))
        (else
         (error "Unknown expression type -- EVAL" exp))))
...
#|
 ...
 | Answer
 |
 | Changing 'actual-value' to 'eval' in the '((application? exp) ...)' clause
 | of the 'cond' expression in the 'eval' procedure allows us to see what
 | happens when we neglect to force the value of the operator.
 |
 | Whenever we use an operator as an operand to a higher-order procedure, all
 | the operands get converted to thunks by the 'list-of-delayed-args' procedure
 | within the '((compound-procedure? procedure) ...)' clause of the 'cond'
 | expression in the 'apply' procedure. If we apply this "procedure", which is
 | actually a thunk, an 'Unknown procedure type -- APPLY (thunk ⟨??⟩ (...))'
 | error occurs.
 |#

(define E0 (setup-environment))
(actual-value '(((lambda (x) x) +) 2 3) E0)
(restart 1)
