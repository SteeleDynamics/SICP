...
#|
 ...
 | rules on queries such as '(last-pair (3) ?x)', '(last-pair (1 2 3) ?x)',
 | and '(last-pair (2 ?x) (3))'. Do your rules work correctly on queries such
 | as '(last-pair ?x (3))'?
 |
 | Answer
 |
 | From the implementation of the 'add-rule!' procedure, the next rule is
 | cons'ed onto the stream of old rules. Therefore, the newer rules are
 | unified and instantiated first.
 |
 | If we assert the 'last-pair' rules in the following order:
 |
 | (assert! (rule (last-pair (?x) (?x))))
 | (assert! (rule (last-pair (?x . ?y) ?z)
 |                (last-pair ?y ?z)))
 |
 | We enter an infinite loop by constantly unifying and instantiating the body
 | of the second rule:
 |
 | Query                  Frame
 | ##########################################################################
 | (last-pair ?x (3))     {}
 | (last-pair ?y (3))     {x: y, z: (3)}
 | (last-pair ?y-1 (3))   {x: y, z: (3), y-1: y, z-1: z}
 | (last-pair ?y-2 (3))   {x: y, z: (3), y-1: y, z-1: z, y-2: y-1, z-2: z-1}
 | ...                    ...
 |#
