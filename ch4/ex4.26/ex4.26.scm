...
; unless? predicate procedure                                   ;!
(define (unless? exp) (tagged-list? exp 'unless))

; unless-condition selector procedure                           ;!
(define (unless-condition exp) (cadr exp))

; unless-usual-value selector procedure                         ;!
(define (unless-usual-value exp) (caddr exp))

; unless-exceptional-value selector procedure                   ;!
...
#|
 ...
 | Alyssa counters that, if one did that, 'unless' would be merely syntax, not
 | a procedure that could be used in conjunction with higher-order procedures.
 | Fill in the details on both sides of the argument. Show how to implement
 | 'unless' as a derived expression (like 'cond' or 'let'), and give an example
 | of a situation where it might be useful to have 'unless' available as a
...
#|
 | 'unless' implemented as a derived expression (metacircular evaluator)
 |#
(define exp0
  '(letrec ((factorial
            (lambda (n) (unless (= n 1) (* n (factorial (- n 1))) 1))))
     (factorial 5)))
(eval exp0 E0)

#|
 | 'unless' implemented as a procedure (underlying Scheme)
 |#
(define (unless condition usual-value exceptional-value)
  (if condition exceptional-value usual-value))
(map unless '(#f #t #f #f) '(1 2 3 4) '(5 6 7 8))
