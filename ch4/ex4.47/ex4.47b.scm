...
; parse-sentence ambiguous procedure
...
; parse-verb-phrase (Louis Reasoner)
(define (parse-verb-phrase)
...
#|
 ...
 |
 | Q1. Does this work?
 | A1. No. Louis Reasoner's production rule infinitely tries to expand
 |     'parse-verb-phrase'. For the sentence '(The cat sleeps), the
 |     nondeterministic evaluator returns the only correct parse tree but
 |     is unable to exhaust the search and enters an infinite loop.
 | Q2. Does the program's behavior change if we interchange the order of
 |     expressions in the 'amb'?
 | A2. Ultimately, No. The production rule infinitely tries to expand
 |     'parse-verb-phrase'. For the previous example, the nondeterministic
 |     evaluator search never finds the correct parse tree and enters an
 |     infinite loop.
 |#

; parse-verb-phrase (Louis Reasoner, reversed)
(define (parse-verb-phrase)
