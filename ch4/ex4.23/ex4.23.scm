...
#|
 | §4.1.7 'Separating Syntactic Analysis form Execution'
 |#
...
#|
 ...
 | Answer:
 |
 | Let ⟨proc₁ proc₂ ... procₙ⟩ be the sequence of analyzed expressions whose
 | values are procedures.
 |
 | In Alyssa's version of 'analyze-sequence', there is an additional 'null?'
 | check for each iteration of looping through the sequence of analyzed
 | procedures. In text version, the looping is already "unrolled" such that
 | the resulting procedure value has the form:
 |
 | (lambda (env)
 |   ...
 |      ((lambda (env)
...
(define exp0
  '(let ((f (lambda (g x) (if (= x 0) 1 (* x (g g (- x 1))))))) (f f 6)))
(eval exp0 E0)
