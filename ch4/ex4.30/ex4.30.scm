...
#|
 ...
 | Answer:
 |
 | a. For Ben Bitdiddle's example, all the calls to 'eval-sequence' have
 | '(first-exp exps)' evaluate to a non-thunk value. Therefore the expression
 | '(eval (first-exp exps) env)' will evaluate correctly.
 |
 | b. See the output below.
 |
 | c. Forcing a non-thunk value returns the value itself.
 |
 | d. I think the implementation of 'eval-sequence' should be the version in
 | the text since it is maximally lazy. Mutation and lazy programming
 | languages do not work well together. Implicit in the idea of mutation is the
 | notion of time (value before and after mutation). Lazy evaluation can "stop
 | time" and prevent additional evaluation from occuring.
 |#

(define exp0 '(define (p1 x) (set! x (cons x '(2))) x))
(define exp1 '(define (p2 x) (define (p e) e x) (p (set! x (cons x '(2))))))
(define exp2 '(p1 1))
(define exp3 '(p2 1))
...
(actual-value exp3 E0)

#|
 | modified 'eval-sequence'
 |#

; eval-sequence procedure (Cy D. Fect)
(define (eval-sequence exps env)
  (cond ((last-exp? exps) (eval (first-exp exps) env))
        (else (actual-value (first-exp exps) env)
              (eval-sequence (rest-exps exps) env))))
...
(actual-value exp0 E1)
(actual-value exp1 E1)
(actual-value exp2 E1)
(actual-value exp3 E1)
