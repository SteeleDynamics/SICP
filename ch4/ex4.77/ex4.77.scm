...
#|
 ...
 | Tasks:
 |
 | 1. Create a new special form 'filter' such that 'not' and 'lisp-value'
 |    are derived queries that use 'filter'.
 |
 | 2. Add 'filter' to 'operation-table', dispatch to 'filter' procedure.
 |
 | 3. Augment 'negate' and 'lisp-value' procedures to create derived
 |    'filter' queries. Then use 'qeval' to evaluate those derived
 |    queries.
 |
 | 4. Append the '(delay (qeval ⟨filter query⟩ frame-stream hist))'
 |    "promise" to all frames resulting from ⟨filter query⟩.
 |
 | 5. Implement '(unbound-var? exp frame)' procedure (uses a locally
 |    defined 'tree-walk' procedure).
 |
 | 6. If the value '(unbound-var? exp frame)' is false, 'force' the
 |    filter promise.
 |
 | 7. Implement check of #6 in 'qeval' procedure.
 |#
