...
#|
 ...
 | nondeterministic procedure that solves this problem based upon generating
 | only those possibilities that are not already ruled out by previous
 ...
 |                   (list 'smith smith))))))))
 |
 | In exercise 4.39, all ambiguous values have scope over the entire body of the
 | 'let' expression. Therefore all combinations of values are examined, with
 | 5⁵ = 3125 leaves in the tree of possible values. Imposing the 'distinct?'
 | restriction, the there are 5! = 120 possible values remaining.
 |
 | The tree of possible values gets pruned at each successive 'let' expression
 | because ambiguous values are examined only over the scope in which they are
 | defined. Therefore unnecessary combinations of values are not examined.
 |
 | See ex4.40.out for Amb-Eval output. This implementation is much more
 | efficient than the implementation in exercise 4.39 (observed running time).
 |#
