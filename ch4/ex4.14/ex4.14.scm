...
#|
 ...
 | version of 'map' as a primitive for the metacircular evaluator. When he
 | tries it, things go terribly wrong. Explain why Louis's map fails even
 | though Eva's works.
 |
 | Answer
 |
 | The system version of 'map' expects a system version of a procedure object
 | as an argument. So when the system version of 'map' is applied to the
 | metacircular evaluator version of a procedure object (and subsequently
 | evaluated), the system can't select the correct parts of the procedure
 | argument in order to apply that procedure to each element in the list.
 |#

; unit tests
(define E0 (setup-environment))
(eval '(map (lambda (x) (+ x 1)) '(0 1 2 3)) E0)
(restart 1)
