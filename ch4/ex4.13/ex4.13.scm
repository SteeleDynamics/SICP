...
; eval-unbound procedure
(define (eval-unbound exp env)                    ;!
  (undefine-variable! (unbound-variable exp) env)
  'ok)
...
; unbound? predicate procedure
(define (unbound? exp)                            ;!
  (tagged-list? exp 'make-unbound!))

; unbound-variable selector procedure
...
; undefine-variable! mutator procedure
  ...
  (let ((frame (first-frame env)))
    (let ((vars (frame-variables frame))
      ...
      (frame-lookup-cps
        ...
        vars
        vals
        (lambda (vals) (set-car! vals '*unbound*))
        (lambda () (display ""))))))
...
#|
 ...
 | special form make-unbound! that removes the binding of a given symbol from
 | the environment in which the make-unbound! expression is evaluated. This
 | problem is not completely specified. For example, should we remove only the
 | binding in the first frame of the environment? Complete the specification
 | and justify any choices you make.
 |
 | Answer
 |
 | Make-unbound! removes the binding of a given symbol from the environment in
 | which the make-unbound! expression is evaluated. That is:
 |
 | 1. Removes the binding of a given symbol only in the first frame, the
 |    environment in which the make-bound! expression is evaluated.
 | 2. Sets value of given symbol to '*unbound*.
 | 3. Lookup of symbol whose associated value is '*unbound* returns an
 |    unbound variable error.
 | 4. The Representation Invariant (RI) for each frame is that the first
 |    occurrence of a given variable is the only bound occurrence. All
 |    subsequent occurrences will have their values set to '*unbound*.
 |
 | Pros:
 | 1. Minimal changes to existing metacircular evaluator
 | 2. Make-unbound! has same time-complexity as define
 | 3. Only unbinding symbol in first frame leaves other code alone
 |
 | Cons: (pun intended)
 | 1. Buildup of unbound symbols in environment
 |
 | Overall, make-unbound! shouldn't be a feature in Scheme because of
 | lexical/static scoping and the semantics of the define expression.
 |#

(define E0 (setup-environment))
(eval '(begin (define foo 0) foo) E0)
(eval '(begin (make-unbound! foo) foo) E0)
(restart 1)
(eval '(begin (define foo 1) foo) E0)
(eval '(define f (lambda (x) (begin (make-unbound! x) 42))) E0)
(eval '(begin (define x 2) (f x) x) E0)
E0
