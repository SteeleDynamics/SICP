...
#|
 ...
 | Answer:
 |
 | Each query operand of the 'disjoin' and 'stream-flatmap' procedure can
 | result in an infinite stream. If the resulting infinite streams were simply
 | appended, then none of the subsequent resulting streams would ever be
 | reached since the first stream is infinte. Instead, the query evaluator
 | calls the 'interleave-delayed' procedure to ensure that frames from all
 | resulting infinite streams are unified against a query.
 |#
