...
; ramb? predicate procedure
(define (ramb? exp) (tagged-list? exp 'ramb))

; ramb-choices selector procedure
(define (ramb-choices exp) (cdr exp))

; permanent? predicate procedure
(define (permanent? exp) (tagged-list? exp 'permanent-set!))

; permanent-variable selector procedure
(define (permanent-variable exp) (cadr exp))

; permanent-value selector procedure
(define (permanent-value exp) (caddr exp))

; if-fail? predicate procedure
...
; if-fail-secondary
(define (if-fail-secondary exp) (caddr exp))
...
(define (analyze exp)
  (cond ((self-evaluating? exp)
        ...
        ((permanent? exp) (analyze-permanent exp))    ;**
...
; analyze-if-fail procedure
(define (analyze-if-fail exp)
  (let ((pproc (analyze (if-fail-primary exp)))
        (sproc (analyze (if-fail-secondary exp))))
    (lambda (env succeed fail)
      (pproc env
             ;; succ cont eval'g prim expr to get prim-val
             (lambda (prim-value fail2)
               (succeed prim-value fail2))
             ;; fail cont eval'g prim expr
             (lambda ()
               (sproc env
                      ;; succ cont eval'g secd expr to get secd-val
                      (lambda (secd-val fail3)
                        (succeed secd-val fail3))
                      ;; fail cont eval'g secd expr
                      fail))))))
...
(define (rand-sel x n)
  (let ((k (random n)))
    (let ((kth (list-ref x k))
          (rest (remove x k)))
      (cons kth rest))))

; randomize default-random-source
(random-source-randomize! default-random-source)

; analyze-ramb procedure
(define (analyze-ramb exp)
  (let ((cprocs (map analyze (ramb-choices exp))))
    (lambda (env succeed fail)
      (define (try-next choices n)
        (if (zero? n)
            (fail)
            (let ((res (rand-sel choices n)))
              ((car res) env
                         succeed
                         (lambda ()
                           (try-next (cdr res) (- n 1)))))))
...
#|
 ...
 | should find that much of the mechanism we built in this section is subsumed
 | by nondeterministic search and backtracking. You will probably also find,
...
(define (query exp)
  (let ((q (query-syntax-process exp)))
    (let ((res (qeval q '()))
          (proc (lambda (v f) (contract-question-mark v))))
      (instantiate q res proc))))

; qeval process
(define (qeval exp frame)
  (extend '(? x)
          '(Bitdiddle Ben)
          (extend '(? type)
                  '(Wizard)
