...
#|
 ...
 | Answer
 |
 | 'Interleave' has a value that is a Lisp-procedure, not a special form.
 | Therefore both stream operands follow applicative-order evaluation. Since the
 | order of evaluation of the operands is not specified in any standard of Lisp,
 | there is a possibility of the second operand being evaluated first, meaning
 | that 'flatten-stream' could evaluate '(stream-cdr stream)' all the way down
 | the stream until the null stream is reached. Therefore, the expression
 | '(flatten-stream stream)' would evaluate to 'the-empty-stream' every time.
 |#
