...
#|
 ...
 | will simply feed the query pattern to 'qeval'. This will produce a stream
 ...
 | as Ben completes the implementation and is about to try it out, Cy walks by,
 ...
 | Answer:
 |
 | Ben just realized that 'accumulation-function' can receive duplicate inputs.
 | To remove elements in '(stream-cdr s)' wrt the value of '(stream-car s)', we
 | follow the 'sieve' procedure example given in §3.5.2 - Infinite Streams. We
 | can recursively filter the stream of frames to remove duplicates before
 | extracting values of the designated variable.
 |#
