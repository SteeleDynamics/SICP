...
;;; stream-cartesian procedure
(define (stream-cartesian streams)                                  ; ***
  (define (proc x)
    (stream-map
      (lambda (y) (cons x y))
      (stream-cartesian (cdr streams))))
   (if (null? streams)
       (singleton-stream the-empty-stream)
       (stream-flatmap proc (car streams))))

;;; resolve procedure
(define (resolve frames)                                            ; ***
  (define (proc bindings acc)
    (if (null? bindings)
        acc
        (let ((binding (car bindings)))
          (let ((var (binding-variable binding))
                (val (binding-value binding)))
            (let ((acc (extend-if-possible var val acc)))
              (if (eq? acc 'failed)
                  '()
                  (proc (cdr bindings) acc)))))))
      (proc (fold-left append '() frames) '()))

;;; conjoin procedure
(define (conjoin conjuncts frame-stream hist)                       ; ***
  (let ((proc (lambda (conj) (qeval conj frame-stream hist))))
    (let ((streams (map proc conjuncts)))
      (let ((prods (stream-cartesian streams)))
        (let ((res (stream-map resolve prods))
              (pred (lambda (x) (not (null? x)))))
          (stream-filter pred res))))))
...
#|
 | Exercise 4.76
 |
 | Our implementation of 'and' as a series combination of queries (figure 4.5)
 ...
 | query. If the data base has N elements, and a typical query produces a number
 | of output frames proportional to N (say N/k), then scanning the data base for
 | each frame produced by the first query will require N²/k calls to the pattern
 | matcher. Another approach would be to process the two clauses of the 'and'
 | separately, then look for all pairs of output frames that are compatible. If
 | each query produces N/k output frames, then this means that we must perform
 | N²/k² compatibility checks -- a factor of k fewer than the number of matches
...
(and (job ?a (computer . ?b))
     (job ?c (accounting . ?d))
     (job ?e (administration . ?f))
     (supervisor ?a ?e)
     (supervisor ?c ?e))
