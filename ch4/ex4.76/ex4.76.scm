  ...
  (if (empty-conjunction? conjuncts)
      ...
      (let ((first (first-conjunct conjuncts))
        ...
        (let ((result (qeval first frame-stream hist)))
          (merge-streams result (conjoin rest frame-stream hist))))))
...
; merge-frames procedure
(define (merge-frames f1 f2)                                        ; ***
  (cond ((eq? f2 'failed) the-empty-stream)
        ((null? f1) (singleton-stream f2))
        (else
         (let ((var (caar f1))
               (val (cdar f1)))
           (let ((result (extend-if-possible var val f2)))
             (merge-frames (cdr f1) result))))))
...
#|
 ...
 | number of output frames proportional to N (say N/k), then scanning the data
 | base for each frame produced by the first query will require N²/k calls to
 | the pattern matcher. Another approach would be to process the two clauses
 | of the 'and' separately, then look for all pairs of output frames that are
...
#|
 | Note: The rules '(great . ?x)' and 'reverse' don't work anymore since the
 | new order of evaluating conjucts doesn't produce the same frame-stream. We
 | gained efficency, but we lost expressiveness.
 |#
