...
#|
 ...
 |
 | A1: Because each new frame has a unique set of consistent bindings among all
 | variables. The frames
 |
 |   frame-1: ((?person-1 (Hacker Alyssa P)) (?person-2 (Fect Cy D)))
 |   frame-2: ((?person-1 (Fect Cy D)) (?person-2 (Hacker Alyssa P)))
 |
 | are such frames. Every variable assignment is consistent with the
 | 'lives-near' rule and is unique among all consistent frames.
 |
 | Q2: Is there a way to find a list of people who live near
 | each other, in which each pair appears only once?
 |
 | A2: Yes, if we combine the 'lives-near' query with another query like
 | 'lisp-value', we can restrict the ordering. See below.
 |#
