...
; scheme-number->complex coercion procedure
(define (scheme-number->complex n)
  (make-complex-from-real-imag (contents n) 0))

; apply-generic procedure (with coercion)
(define (apply-generic op . args)
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
      (if proc
...
; install scheme-number->complex procedure into coercion table
...
; (exp z1 z2) --> infinite loop

;
; Part (b)
; ########
; Louis Reasoner is correct. Currently, procedure apply-generic assumes that the
; reason why the entry is missing from the operation-table is because the
; argument types are mismatched, not because the generic procedure is missing
; from the operation-table for equal argument types.
