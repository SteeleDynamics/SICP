...
;
; The reason why adding this code works is because it accomplishes a
; double-dispatch. The first dispactch from apply-generic selects the generic
; procedure magnitude associated with types (complex). The selected generic
; procedure magnitude is installed into the operation-table for types
; (rectangular) and (polar). Then the second dispatch from apply-generic selects
; the magnitude procedure associated with types (rectangular) or (polar). In the
; case of this problem, the dispatch was on types (rectangular).
; 
