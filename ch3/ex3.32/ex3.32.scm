...
#|
 ...
 | #######
 |
 | The FIFO order must be used because the final states (signal values) of wires
 | A and B are correct. If LIFO order was used. The states would be opposite of
 | what was expected. For the and-gate example, the final signal values for A
 | and B would be 0 and 1 respectively, not 1 and 0 respectively as expected.
 |#
...
(after-delay 0 (lambda () (set-signal! A 0)))
(after-delay 0 (lambda () (set-signal! B 1)))
(after-delay 0 (lambda () (set-signal! A 1)))
(after-delay 0 (lambda () (set-signal! B 0)))
(propagate)
